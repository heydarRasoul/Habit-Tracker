...
def auth_token_add():
    ...
    if user_query:
        ...
        try:
            ...
            db.session.rollback()
            return jsonify({"message":"unable to add record."})
        
        return jsonify({"message":"auth seccess", "auth_info": auth_token_schema.dump(new_token)}), 201
...
def auth_token_delete(auth_info):
    try:
        ...
        return jsonify({"message":"logout seccessful"}),200
    except:
        db.session.rollback()
