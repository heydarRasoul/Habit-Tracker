...
class HabitReminders(db.Model):
    ...
    user_id = db.Column(UUID(as_uuid=True), db.ForeignKey("HabitReminders.user_id"), nullable=False)
    habit_id = db.Column(UUID(as_uuid=True), db.ForeignKey("HabitReminders.habit_id"), nullable=False)
...
class HabitRemindersSchema(ma.Schema):
    ...
    reminder_time=ma.fields.Strings(required=True)
